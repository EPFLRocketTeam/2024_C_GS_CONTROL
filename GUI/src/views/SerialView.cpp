...
SerialView::SerialView(QWidget *parent) : QFrame(parent) {
...
SerialView::~SerialView() {
  // Unsubscribe from ClientManager before destruction
  if (MainWindow::clientManager) {
    MainWindow::clientManager->unsubscribeAll(GUI_FIELD::SERIAL_STATUS);
    MainWindow::clientManager->unsubscribeAll(GUI_FIELD::SERIAL_NAME_USE);
  }
}
...
void SerialView::setupConnections() {
  ...
  MainWindow::clientManager->subscribe(GUI_FIELD::SERIAL_STATUS,
                                       setSerialStatus);
  MainWindow::clientManager->subscribe(GUI_FIELD::SERIAL_NAME_USE,
                                       setSerialName);
...
void SerialView::updateStatus(QString newStatus) {
  if (newStatus == "open") {
    ...
    throw std::runtime_error(
        "Invalid argument passed in function \"SerialView::updateStatus\", "
