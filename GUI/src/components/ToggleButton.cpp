...
ToggleButton::ToggleButton(GUI_FIELD fieldSensitivity, QWidget *parent)
    : QWidget(parent), m_checked(false), m_offset(0),
      m_fieldSensitivity(fieldSensitivity) {
  m_state = Unknown;
  ...
  connect(m_timer, &QTimer::timeout, this,
          &ToggleButton::updateAnim); // Connect the timeout signal to the
                                      // update slot
  ...
  connect(this, &ToggleButton::toggled, this, [this](bool checked) {
    ...
    _logger.debug("Toggle Button Clicked",
                  QString(R"(The button for valve %1 was clicked)")
                      .arg(fieldUtil::enumToFieldName(m_fieldSensitivity))
                      .toStdString());
...
int ToggleButton::fieldSensitivity() {
  return static_cast<int>(m_fieldSensitivity);
}
...
QSize ToggleButton::sizeHint() const { return QSize(60, 30); }

bool ToggleButton::isChecked() const { return m_state == Open; }
...
void ToggleButton::setChecked(bool checked) {
  ...
  if (m_state != newState) {
    ...
    animateToggle(200);
    setUnknown(); // Set the state to unknown to indicate that the state is
                  // being updated
...
void ToggleButton::paintEvent(QPaintEvent *event) {
  ...
  if (m_checked && m_loadingMovie->isValid() &&
      m_loadingMovie->currentPixmap().isNull() == false) {
    // Draw the current frame of the loading movie if the button is checked
    painter.drawEllipse(m_offset, 0, height(), height());

  } else {
    // Draw the switch normally if the button is not checked or the loading
    // movie is not valid
    painter.drawEllipse(m_offset, 0, height(), height());
  }
  if (m_state == Unknown) {
    painter.drawPixmap(m_offset + 2, 2, height() - 4, height() - 4,
                       m_loadingMovie->currentPixmap());
  }
}

void ToggleButton::setUnknown() { m_state = Unknown; }
...
qreal ToggleButton::offset() const { return m_offset; }

void ToggleButton::setOffset(qreal offsetValue) { m_offset = offsetValue; }
