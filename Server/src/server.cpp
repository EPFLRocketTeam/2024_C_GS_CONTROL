...
void Server::openSerialPort() {
    ...
    foreach (const QSerialPortInfo &info, QSerialPortInfo::availablePorts()) {
      if (info.portName().startsWith("ttyS")) {
        ...
        _serverLogger.debug(
            "Serial Opening",
            QString(
                R"(name=%4, manufacturer=%1, pId=%2, vendoreId=%3, isNull=%5)")
  ...
  if (serialPort->open(QIODevice::ReadWrite)) {
    ...
    _serverLogger.info("SerialActions",
                       QString(R"(Could not open serila port %1)")
                           .arg(serial_port_name)
                           .toStdString());
  }
#endif
...
void Server::incomingConnection(qintptr socketDescriptor) {
  ...
  _serverLogger.info(
      "Client Connected",
      QString(R"(Client on socket %1)").arg(socketDescriptor).toStdString());
}

void Server::receiveSubscribe(const QJsonObject &request,
                              QTcpSocket *senderSocket) {
  // Handle the subscribe request
  int field = request.value("payload").toObject().value("field").toInt();

  _serverLogger.info("Subscription",
                     QString(R"(The client %1 is trying to subscribe to %2)")
                         .arg((qintptr)senderSocket)
                         .arg(fieldUtil::enumToFieldName((GUI_FIELD)field))
                         .toStdString());

  if (subscriptionMap[field].contains(senderSocket)) {
    _serverLogger.warn("Subscription failed",
                       QString(R"(The client %1 is already subscribed to %2)")
                           .arg((qintptr)senderSocket)
                           .arg(fieldUtil::enumToFieldName((GUI_FIELD)field))
                           .toStdString());
    return;
  ...
  _serverLogger.debug("Subscription Successfull",
                      QString(R"(The client %1 subscribed to %2)")
                          .arg((qintptr)senderSocket)
                          .arg(fieldUtil::enumToFieldName((GUI_FIELD)field))
                          .toStdString());
}

void Server::receiveUnsubscribe(const QJsonObject &request,
                                QTcpSocket *senderSocket) {
  // Handle the unsubscribe request
  int field = request.value("payload").toObject().value("field").toInt();
  subscriptionMap[field].removeOne(senderSocket);
  _serverLogger.debug("Unsubscribed Successfully",
                      QString(R"(The client %1 unsubscribed from %2)")
                          .arg((qintptr)senderSocket)
                          .arg(fieldUtil::enumToFieldName((GUI_FIELD)field))
                          .toStdString());
}
...
void Server::receiveGet(const QJsonObject &request, QTcpSocket *senderSocket) {
  ...
  _serverLogger.debug("Received Get Request",
...
void Server::receivePost(const QJsonObject &request, QTcpSocket *senderSocket) {
  ...
  QJsonDocument doc(request);
  _serverLogger.debug("Received Post Request",
                      doc.toJson(QJsonDocument::Indented).toStdString());
  ...
  while (senderSocket->bytesAvailable() > 0) {
    ...
    for (QString &jsonStr : jsonStrings) {
      if (counter == 0 && jsonStrings.length() > 1) {
        jsonStr.append("}");
      } else if (jsonStrings.length() - 1 == counter &&
                 jsonStrings.length() > 1) {
        jsonStr.prepend("{");
...
void Server::disconnected() {
  ...
  _serverLogger.info("ServerConnections",
                     QString(R"(A Clien disconnect with socket %1)")
                         .arg(senderSocket->socketDescriptor())
                         .toStdString());
...
int Server::sendDataToSocket(QTcpSocket *socket, RequestBuilder request) {
  ...
  _packetLogger.debug("Sent packets to client",
                      request.toString().toStdString());
...
void Server::updateSubscriptions(const QJsonObject &newData) {
  ...
  for (auto it = newData.constBegin(); it != newData.constEnd(); ++it) {
    QRegularExpression regex("^-?\\d+$"); // Regular expression to match
    ...
    if (value.isObject()) {
      ...
      for (QTcpSocket *socket : sockets) {
        rBuilder.addField(it.key(),
                          QString(QJsonDocument(value.toObject()).toJson()));
        QByteArray data = rBuilder.toString().toUtf8();
        _packetLogger.debug("Sent packets to client",
                            rBuilder.toString().toStdString());
        socket->write(data, data.size());
        socket->waitForBytesWritten();
        socket->flush();
      }
...
void Server::handleCommand(const QJsonObject &command) {
  ...
  case GUI_FIELD::SERIAL_STATUS: {
    ...
    const QString &serialStatus =
        QString(serialPort->isOpen() ? "open" : "close");
    m_payload[QString::number(GUI_FIELD::SERIAL_STATUS)] = serialStatus;
    _serverLogger.info("Send Response",
                       QString(R"(The response to Serial Status is %1)")
                           .arg(QString(QJsonDocument(m_payload).toJson()))
                           .toStdString());
  ...
  case GUI_FIELD::SERIAL_NAME_USE: {
    QJsonObject m_payload;
    m_payload[QString::number(GUI_FIELD::SERIAL_NAME_USE)] =
        serialPort->portName();
    updateSubscriptions(m_payload);
    break;
  }
  ...
  default:
    ...
    _serverLogger.info("Received Command",
                       QString(R"(Send command for %1 with value %2)")
                           .arg(fieldUtil::enumToFieldName((GUI_FIELD)f))
                           .arg(order)
                           .toStdString());
...
void Server::sendSerialPacket(uint8_t packetId, uint8_t *packet,
                              uint32_t size) {
  ...
  if (serialPort->isOpen()) {
    ...
    _serverLogger.error(
        "Serial Send",
        "The serial port is not opened, packet couldn't be send");
  }
}

void Server::abort_loop() {
  if (abort_initiated)
    return;
  auto *timer = new QTimer(this);
  connect(timer, &QTimer::timeout, this, [this]() {
    av_uplink_t *packet = new av_uplink_t;
    int capsule_id = createUplinkPacketFromRequest(GUI_CMD_ABORT, 1, packet);
    sendSerialPacket(capsule_id, (uint8_t *)packet, av_uplink_size);
  });
  timer->start(1000);
...
void Server::handleSerialPacket(uint8_t packetId, uint8_t *dataIn,
                                uint32_t len) {
  ...
  if (result && result->contains("ABORT")) {
    ...
    _packetLogger.error("Parsing Error",
                        QString(R"(The message with ID=%1 could not be parsed)")
                            .arg(packetId)
                            .toStdString());
  }
}
...
void Server::simulateJsonData() {
  ...
  /**/
  /*jsonObj[QString::number(GUI_FIELD::SERIAL_STATUS)] =
   * QString(serialPort->isOpen() ? "open" : "close");*/
  /*jsonObj["1234"] = 50;*/
  /**/
  ...
  std::uniform_real_distribution<float> distCoord(-180.0f,
                                                  180.0f); // for lon/lat
